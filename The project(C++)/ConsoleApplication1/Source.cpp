...
//this Project made By * Hazem & Yazan & Nazeer & Ramy * 
...
float MASS= 1.0f	;			// default point mass
float BALLRADIUS= 1.0f;		    // default radius of the ball
float KS =1755.0f;				// spring constant
float KD= 35.0f;				// spring damping constant
float FINAL_PRESSURE = 99.0f ;
...
int mousedown = 0;
float xMouse, yMouse;
int closest_i = 1;				// the closest point to mouse click
...
int main(int argc,char*argv[])
{
	...
	glutCreateWindow("Soft Body 2D ");
	KS=atof(argv[1]);
	KD=atof(argv[2]);
	MASS=atof(argv[3]);
    BALLRADIUS=atof(argv[4]);
	FINAL_PRESSURE=atof(argv[5]);
	std::cout<<BALLRADIUS;
	std::cout<<MASS;
	std::cout<<KS;
	std::cout<<KD;
    std::cout<<FINAL_PRESSURE ;
	CreateBall();
